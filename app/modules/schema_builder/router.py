...
from app.modules.schema_builder.schema import (
...
from app.modules.schema_builder.service import SchemaService
from app.modules.schema_builder.repository import (
...
async def get_schema(
    ...
    project_service: ProjectService = Depends(lambda db=Depends(get_db): ProjectService(ProjectRepository(db))),
):
    # Vérifier que l'utilisateur peut accéder au projet
...
@router.patch(
    ...
    description="Modifie les propriétés d'un champ (type, osbligatoire, unique, configuration, etc.)."
)
