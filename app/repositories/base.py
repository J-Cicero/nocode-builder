...
class BaseRepository(Generic[ModelType, CreateSchemaType, UpdateSchemaType]):
    ...
    async def get_by_uuid(self, db: AsyncSession, uuid: str) -> Optional[ModelType]:
        result = await db.execute(select(self.model).filter(self.model.uuid == uuid))
        return result.scalars().first()
    ...
    async def remove_by_uuid(self, db: AsyncSession, *, uuid: str) -> ModelType:
        obj = await self.get_by_uuid(db, uuid)
        if obj:
            await db.delete(obj)
            await db.commit()
        return obj
